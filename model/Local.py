...
class Local(AbstractRecommender):
    def __init__(self, sess, dataset, config):
        super(Local, self).__init__(dataset, config)
    ...
    def get_adj_mat(self):
        ...
        for cluster in range(self.localmodel):
                random_adj[cluster] = sp.load_npz('dataset/%s/100%s/s_random_adj_mat%d.npz'%(self.data_name,self.adj_type,cluster))
    ...
    def _create_lightgcn_embed(self,adj):
    ...
    def build_graph(self):
        ...
        *********************************************************
        ...
        for n_cluster in range(self.localmodel):
            self.all_embeddings_temp = self._create_lightgcn_embed(self._convert_sp_mat_to_sp_tensor(self.norm_adj[n_cluster]))
            if n_cluster==0:
                self.all_embeddings=[self.all_embeddings_temp]
                continue
            self.all_embeddings += [self.all_embeddings_temp]
        self.all_embeddings = tf.stack(self.all_embeddings,1)
        self.anchor_emb =tf.reduce_mean(self.all_embeddings,axis=0,keepdims=False)
        self.anchor_out=self.kernel(self.anchor_emb,self.localmodel**2)

        self.all_embeddings = tf.matmul(self.anchor,tf.reshape(tf.transpose(self.all_embeddings,[1,0,2]),[self.localmodel,-1]))
        self.all_embeddings = tf.reduce_mean(self.all_embeddings,axis=0,keepdims=False)
#         self.all_embeddings = tf.reduce_max(self.all_embeddings,axis=0,keepdims=False)
#         self.all_embeddings = tf.nn.l2_normalize(self.all_embeddings)
    ...
    def train_model(self):
        ...
        for epoch in range(self.epoch):
            start = time()
            if epoch<100:
                _, loss, anchor = self.sess.run([self.opt, self.loss,self.anchor_out],feed_dict={self.anchor:self.w})
                self.w=anchor
                end = time()
                print('ave_epoch%d: loss = %f time = %fs' % (epoch, loss,end - start))
            else:
