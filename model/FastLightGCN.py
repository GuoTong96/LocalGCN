...
class FastLightGCN(AbstractRecommender):
    def __init__(self, sess, dataset, config):
        ...
        self.logger.info("\"num_users\": %d,\"num_items\":%d, \"num_ratings\":%d, \"sparsity\":%.4f"%(self.n_users, self.n_items,self.num_ratings,self.sparsity))
    ...
    def _create_lightgcn_embed(self,ego_embeddings):
    ...
    def build_graph(self):
        ...
        *********************************************************
        ...
        if self.alg_type in ['lightgcn']:
            self.ego_embeddings = tf.concat([self.weights['user_embedding'], self.weights['item_embedding']], axis=0)

            self.all_embeddings = self._create_lightgcn_embed(self.ego_embeddings)
        ...
        *********************************************************
        ...
        self.emb=tf.concat([self.user_embeddings_final,self.item_embeddings_final],0)
    ...
    def train_model(self):
        ...
        for epoch in range(self.epoch):
            ...
            if epoch % 20 == 0:
                ...
                if best<pre:
                    best=pre
                    bestresult=self.sess.run(self.ego_embeddings)
                    ...
                    if n>=10:
                        ...
                        break
#         np.save('%s_eu'%self.data_name,eu)
    ...
    def predict(self, user_ids, items=None):
        ...
        if user_ids[0]==14336:
            np.save('batch.npy',user_ids)
            np.save('rate.npy',i_rate_batch)
        return i_rate_batch
