...
from util import DataGenerator, Learner, Tool, create_logger
...
#                                                              time()-training_start_time))
    ...
    def _create_ngcf_embed(self):
        ...
        for k in range(0, self.n_layers):
            ...
            side_embeddings=tf.sparse_tensor_dense_matmul(self._convert_sp_mat_to_sp_tensor(self.norm_adj), ego_embeddings)
...
#             adj_mat = self.normalized_adj_single(A + sp.eye(A.shape[0]))
            adj_mat=sp.load_npz('dataset/%s/norm_adj_mat.npz'%self.data_name)
